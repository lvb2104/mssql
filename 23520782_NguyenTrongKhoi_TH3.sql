...
--Câu 2:Thêm vào thuộc tính GHICHU có kiểu dữ liệu varchar(20) vào quan hệ SANPHAM
ALTER TABLE SANPHAM
...
ALTER TABLE SANPHAM ADD CONSTRAINT Check_don_vi_tinh CHECK (DVT IN ('cay','hop','cai','quyen','chuc'));
...
ALTER TABLE HOADON ADD CONSTRAINT check_so_luong CHECK (SOHD >=1)

--Câu 10:Ngày khách hàng đăng ký là khách hàng thành viên phải lớn hơn ngày sinh của người đó.
ALTER TABLE KHACHHANG ADD CONSTRAINT check_ngay_dang_ki CHECK (NGDK > NGSINH)
...
--Câu 2: Tạo quan hệ SANPHAM1 chứa toàn bộ dữ liệu của quan hệ SANPHAM.
...
--Câu 4: Cập nhật giá giảm 5% đối với những sản phẩm do
--“Trung Quoc” sản xuất có giá từ 10.000 trở xuống (cho quan hệ SANPHAM1).
UPDATE SANPHAM1
SET GIA = GIA * 95/100
WHERE NUOCSX = 'Trung Quoc' AND GIA <= 10000


--Câu 5: Cập nhật giá trị LOAIKH là “Vip” đối với những khách hàng đăng ký thành viên 
...
--Câu 7: In ra các số hóa đơn, trị giá hóa đơn trong tháng 1/2007, 
...
WHERE NGHD = '1/1/2007'
--Câu 9: In ra số hóa đơn, trị giá các hóa đơn do nhân viên có tên “Nguyen Van B” lập 
...
--Câu 10: In ra danh sách các sản phẩm (MASP,TENSP) được khách hàng có tên “Nguyen Van A” 
...
--Câu 11: Tìm các số hóa đơn đã mua sản phẩm có mã số san pham “BB01” hoặc “BB02”.
SELECT HOADON.SOHD
FROM HOADON
JOIN CTHD ON HOADON.SOHD = CTHD.SOHD
WHERE CTHD.MASP IN ('BB01','BB02')

--Câu 12: Tìm các số hóa đơn đã mua sản phẩm có mã số “BB01” hoặc “BB02”,
...
--Câu 13: Tìm các số hóa đơn mua cùng lúc 2 sản phẩm có mã số “BB01” và “BB02”,
--mỗi sản phẩm mua với số lượng từ 10 đến 20.
SELECT SOHD
FROM CTHD
...
--Câu 14: In ra danh sách các sản phẩm (MASP,TENSP) do “Trung Quoc” sản xuất 
...
--Câu 17: In ra danh sách các sản phẩm (MASP,TENSP) do “Trung Quoc” sản xuất 0
...
ALTER TABLE HOCVIEN ADD CONSTRAINT check_gioi_tinh CHECK(GIOITINH IN ('Nam','Nu'))
ALTER TABLE GIAOVIEN ADD CONSTRAINT check_gioi_tinh1 CHECK(GIOITINH IN ('Nam','Nu'))

--Câu 4:Điểm số của một lần thi có giá trị từ 0 đến 10 và cần lưu đến 2 số lẽ (VD: 6.22).
ALTER TABLE KETQUATHI ALTER COLUMN DIEM DECIMAL(10,2)
ALTER TABLE KETQUATHI ADD CONSTRAINT check_diem_thi CHECK( DIEM >=0 AND DIEM <=10 )

--Câu 5: Kết quả thi là đạt nếu kết quả thi từ 5 đến 10 và không đạt nếu nhỏ hơn 5
...
ADD CONSTRAINT check_lan_thi CHECK(LANTHI <=3)
--Câu 7: Học kì chỉ có giá trị từ 1 đến 3\
ALTER TABLE GIANGDAY 
ADD CONSTRAINT check_hoc_ki CHECK (HOCKY >=1 AND HOCKY <=3)
--Câu 8: Học vị của giáo viên chỉ có thể là “CN”, “KS”, “Ths”, ”TS”, ”PTS”.
ALTER TABLE GIAOVIEN
ADD CONSTRAINT check_hoc_vi CHECK (HOCVI IN ('CN', 'KS', 'Ths', 'TS', 'PTS'))
...
--2.	Cập nhật giá trị điểm trung bình tất cả các môn học (DIEMTB) của mỗi học viên (tất cả các môn học đều có hệ số 1 và
--nếu học viên thi một môn nhiều lần, chỉ lấy điểm của lần thi sau cùng).
-- Cập nhật DIEMTB trong bảng HOCVIEN
--ALTER TABLE HOCVIEN
--ADD GHICHU VARCHAR(20)
...
SET DIEMTB = (
    ...
    FROM (
        ...
        FROM KETQUATHI KQ
        INNER JOIN (
            -- Subquery để lấy lần thi cuối cùng của mỗi môn cho mỗi học viên
            SELECT MAHV, MAMH, MAX(LANTHI) AS LANTHI
            FROM KETQUATHI
            GROUP BY MAHV, MAMH
        ) AS LATEST
        ON KQ.MAHV = LATEST.MAHV 
           AND KQ.MAMH = LATEST.MAMH 
           AND KQ.LANTHI = LATEST.LANTHI
    ) AS DIEM_CUOI
    WHERE DIEM_CUOI.MAHV = HOCVIEN.MAHV
);
SELECT DIEMTB from hocvien;
...
WHERE KETQUATHI.MAMH = 'CSDL' AND KETQUATHI.DIEM<5 AND KETQUATHI.LANTHI = 1 AND NOT EXISTS(
	SELECT 1
	FROM KETQUATHI KQ2
	WHERE KQ2.MAHV = KETQUATHI.MAHV AND KQ2.MAMH = 'CSDL' AND KQ2.LANTHI >1
)
